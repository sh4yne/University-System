...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$email || !$password) {
        ...
        if ($chk->fetch()) {
            ...
            db()->prepare("INSERT INTO users (name,email,password,role) VALUES (?,?,?,'student')")
                ->execute([$name, $email, $hash]);
            $success = 'Account created! You can now log in. Note: your account will need to be linked to a student record by the Registrar.';
        }
    }
}
?>
...
<div class="login-wrap">
  <div class="login-card">
    ...
    <div class="login-link">
      Already have an account? <a href="/university/login.php">Sign in</a>
    </div>
  </div>
</div>
</body>
